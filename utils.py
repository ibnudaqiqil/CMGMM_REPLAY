...
def samples_to_np_images(samples, image_shape=[64, 64, 3], clamp=True):
    ...
    assert len(image_shape) == 2 or (
        len(image_shape) == 3 and image_shape[2] > 1)
...
def samples_to_mosaic(samples, image_shape=[64, 64, 3]):
    ...
    for i in range(num_images // num_cols):
        rows.append(np.hstack([images[j]
...
def visualize_model(model: MFA, image_shape=[64, 64, 3], start_component=0, end_component=None):
    ...
    if len(image_shape) == 3:
        canvas = np.ones(
            [(l+1)*(h+1), k*(w*3+2) + (k-1)*spacer, image_shape[2]])
    else:
    ...
    for c_num in range(start_component, end_component):
        ...
        canvas[:h, x_start+w//2+w+2:x_start+w //
               2+2*w+2] = to_im(D / torch.max(D))

        for i in range(l):
            y_start = (i+1)*(h+1)
            A_i = model.A[c_num, :, i]
            canvas[y_start:y_start+h, x_start:x_start+w] = to_im(mu + z * A_i)
            canvas[y_start:y_start+h, x_start+w +
                   1:x_start+2*w+1] = to_im(0.5 + z * A_i)
            canvas[y_start:y_start+h, x_start+2*w +
                   2:x_start+3*w+2] = to_im(mu - z * A_i)
    return canvas
