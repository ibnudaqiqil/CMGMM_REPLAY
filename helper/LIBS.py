...
def permute_mnist(num_task, batch_size):
    ...
    for i in range(num_task):
        ...
        train_PMNIST_DataLoader = PermutedMNISTDataLoader(
            train=True, shuffle_seed=shuffle_seed)
        test_PMNIST_DataLoader = PermutedMNISTDataLoader(
            train=False, shuffle_seed=shuffle_seed)
    ...
    for testloader in test_dataloaders:
        for image, _ in testloader:
            ...
            with torch.autograd.no_grad():
                ...
                p_fake += (torch.sum(disc(gen(sample_noise(batch_size, num_noise)))).item()) / \
                    10000.
...
def solver_evaluate(cur_task, gen, solver, ratio, device, TestDataLoaders):
    ...
    # solver_loss = 0.0
    celoss = torch.nn.CrossEntropyLoss().to(
        device) if torch.cuda.is_available() else torch.nn.CrossEntropyLoss()
