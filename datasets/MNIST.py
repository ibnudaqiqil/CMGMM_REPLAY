...
class MNIST_PermutedDataset(torchvision.datasets.MNIST):
    ...
    def __init__(self, source='data/mnist_data', train = True, shuffle_seed = None):
        super(MNIST_PermutedDataset, self).__init__(
            source, train, download=True)
...
class MNIST_IncrementalDataset(torchvision.datasets.MNIST):
    ...
    def appendData(self,psudodata,label):
        for i in range(len(psudodata)):
            _data = self.transform(psudodata[i].numpy()) if self.transform is not None else psudodata            
            self.TrainData.append(_data)
            self.TrainLabels.append(label[i])
