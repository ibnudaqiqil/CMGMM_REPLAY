...
class IGMN():
    ...
    def updateComponents(self, x):
        for i in range(self.nc):
            ...
            if self.fullcovs:
                ...
                self.covs[i] = self.covs[i] - deltaMU**2 + w * \
                    (xcenteredNEW**2 - self.covs[i]) + self.regVal
    ...
    def train(self, X):
        ...
        for i in range(N):
            if (i > 0 and i % 100 == 0):
                print 'processing data %d/%d -- num comps: %d' % (i, N, self.nc)
        ...
        for j in range(N):
            ...
            for i in range(self.nc):
                ...
                if self.fullcovs:
                    ...
                    xm[i, :] = meanB + \
                        np.dot(covBA, linalg.solve(covA,  x - meanA))
                else:
